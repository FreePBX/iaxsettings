...
while (isset(${$var_iax_custom_key}))
{
	...
	{
		$iaxcinputs .= sprintf('addCustomField("%s","%s");', ${$var_iax_custom_key}, ${$var_iax_custom_val});		
	}
	$idx++;
	$var_iax_custom_key = "iax_custom_key_$idx";
	$var_iax_custom_val = "iax_custom_val_$idx";
...
foreach ($codecs as $codec => $codec_state)
{
	...
	$rows[$count] = sprintf('<li><a href="#"><b>%s&nbsp;</b><input type="checkbox" name="codec[%s]" id="%s" value="1" %s></a></li>', $codec_trans, $codec, $codec, $codec_checked);
}
ksort($rows);
...
foreach ($video_codecs as $codec => $codec_state)
{
	...
	$codec_checked = $codec_state ? 'checked' : '';
	$count = !empty($codec_state) ? $codec_state : $c++;
	$rows[$count] = sprintf('<li><a href="#"><b>%s&nbsp;</b><input type="checkbox" name="vcodec[%s]" id="%s" value="1" %s></a></li>', $codec_trans, $codec, $codec, $codec_checked);
}
ksort($rows);
$vcodeclist = implode(PHP_EOL, $rows);
unset($rows);
?>
							...
							<div class="form-group">
								...
								<div class="col-md-9 form-horizontal">
									<div class="row">
										<div class="col-md-12">
											<div class="row">
												<div class="col-md-5"><label class="control-label"><?php echo _("Setting") ?></label></div>
												<div class="col-md-6"><label class="control-label"><?php echo _("Value") ?></label></div>
												<div class="col-md-1">
													<button type="button" id="iax-custom-add" class="btn btn-success btn-block" title="<?php echo _("Add New Setting"); ?>"><i class="fa fa-plus" aria-hidden="true"></i></button>
												</div>
											</div>
										</div>
									</div>
									<div class="row">
										<div class="col-md-12" id="iax-custom-buttons">
											<!-- Now the lines are loaded with js to avoid duplicating code -->
										</div>
									</div>
								</div>
							</div>
			...
			<div class="element-container">
				<div class="row">
					<div class="col-md-12">
									...
									<div class="row">
										<div class="col-md-12">
											<div class="row">
												<div class="form-group">
													<div class="col-md-12">
														<label class="control-label"><?php echo _("Audio Codecs") ?></label>
													</div>
												</div>
											</div>
										</div>
						...
						<div class="row">
							<div class="col-md-6">
								<!--Audio Support List Codec-->
								<ul class="sortable" id="acodeclist">
									<?php echo $acodeclist?>
								</ul>
								<!--END Audio Support List Codec-->
							</div>
							<div class="col-md-6">
								<!--Video Support List Codec-->
								<div class="video-codecs">
									<ul class="sortable video-codecs" id="vcodeclist">
										<?php echo $vcodeclist?>
									</ul>
								</div>
								<!--END Video Support List Codec-->
							</div>
						</div>
